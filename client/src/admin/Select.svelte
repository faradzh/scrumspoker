<script lang="ts">
    ...
    const errorClassName = $derived(error ? 'select-error' : '');
</script>
...
<select class={`select select-bordered ${errorClassName} w-full h-9 min-h-9 rounded-md`} bind:value={value} disabled>
